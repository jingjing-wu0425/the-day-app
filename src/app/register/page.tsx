    ...
    <div className="min-h-dvh bg-bg flex flex-col items-center justify-center px-8">
      ...
      <div className="w-full max-w-xs space-y-4">
        ...
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="密码（至少6位）"
          className="w-full bg-fg/5 rounded-xl px-4 py-3 text-[15px] text-fg/80 placeholder:text-muted/30 outline-none font-light" />
        <input type="text" value={nickname} onChange={(e) => setNickname(e.target.value)} placeholder="昵称"
      ...
      <button onClick={() => router.push("/login")} className="mt-6 text-[13px] text-muted/50 font-light hover:text-fg/60 transition-colors">
        已有账号？登录
      </button>
    </div>
  );
